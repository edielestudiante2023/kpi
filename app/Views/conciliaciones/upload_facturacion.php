...
<head>
    ...
    <title>Cargar Facturación – Conciliaciones – Kpi Cycloid</title>
...
<div class="container py-4">
    <div class="d-flex align-items-center gap-2 mb-4">
        ...
        <h1 class="h3 mb-0">Cargar Tabla de Facturación</h1>
    </div>
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4">
            <div class="card text-center">
                <div class="card-body">
                    <h5 class="card-title text-muted">Portafolios</h5>
                    <p class="display-6 fw-bold"><?= count($portafolios ?? []) ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="<?= base_url('conciliaciones/facturacion/upload') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="mb-3">
                    ...
                    <div class="form-text">
                        Suba el archivo "TABLA DE FACTURACION.xlsx" tal como está. El sistema mapea el portafolio automáticamente.
                    </div>
                </div>
    ...
    <div class="d-flex gap-2">
        <a href="<?= base_url('conciliaciones/facturacion') ?>" class="btn btn-outline-primary">
            <i class="bi bi-table me-1"></i> Ver registros importados
        </a>
        <?php if (($totalRegistros ?? 0) > 0): ?>
        <button class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#modalTruncar">
            <i class="bi bi-trash me-1"></i> Vaciar tabla para reimportar
        </button>
        <?php endif; ?>
    </div>
</div>

<!-- Modal confirmar truncar -->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Esto eliminará <strong>todos los <?= number_format($totalRegistros ?? 0, 0, ',', '.') ?> registros</strong> de la tabla de facturación.</p>
                <p class="text-danger fw-bold">Esta acción no se puede deshacer.</p>
            </div>
            <div class="modal-footer">
                <button class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <a href="<?= base_url('conciliaciones/facturacion/truncar') ?>" class="btn btn-danger">
                    Sí, vaciar tabla
                </a>
            </div>
        </div>
