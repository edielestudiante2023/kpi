...
<head>
    ...
    <title>Nueva empresa – CRM – Kpi Cycloid</title>
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h5 mb-0"><i class="bi bi-building-add me-2"></i>Nueva empresa</h1>
    ...
    <form method="post" action="<?= base_url('crm/empresas/nueva') ?>" class="card shadow-sm">
        <?= csrf_field() ?>
        <div class="card-body">
            <div class="row g-2">
                <div class="col-md-8">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="nit" class="form-control form-control-sm" value="<?= old('nit') ?>">
                </div>

                <div class="col-md-4">
                    <label class="form-label small">Sector</label>
                    <input type="text" name="sector" class="form-control form-control-sm"
                           value="<?= old('sector') ?>" placeholder="Ej: SST, PH, Talento">
                </div>
                <div class="col-md-4">
                    ...
                    <select name="tamano" class="form-select form-select-sm">
                        ...
                        <?php foreach (['micro' => 'Micro', 'pequena' => 'Pequeña', 'mediana' => 'Mediana', 'grande' => 'Grande'] as $v => $l): ?>
                            <option value="<?= $v ?>" <?= old('tamano') === $v ? 'selected' : '' ?>><?= $l ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label small">Ciudad</label>
                    <input type="text" name="ciudad" class="form-control form-control-sm" value="<?= old('ciudad') ?>">
                </div>
                ...
                <div class="col-md-6">
                    <label class="form-label small">Responsable (dueño de la cuenta)</label>
                ...
                <div class="col-12 form-check ms-2 mt-2">
                    <input type="checkbox" name="activo" value="1" class="form-check-input" id="activo"
                           <?= (int) old('activo', 1) === 1 ? 'checked' : '' ?>>
                    <label class="form-check-label small" for="activo">Empresa activa</label>
                </div>
            </div>
        </div>
        <div class="card-footer text-end">
            <a href="<?= base_url('crm/empresas') ?>" class="btn btn-secondary btn-sm">Cancelar</a>
