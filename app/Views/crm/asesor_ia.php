...
<head>
    ...
    <title>OTTO Coach Comercial – CRM – Kpi Cycloid</title>
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
            <h1 class="h4 mb-0 d-flex align-items-center gap-2">
                ...
                OTTO Coach Comercial
                <i class="bi bi-robot ms-1"></i>
            </h1>
            <small class="text-muted">
                Asistente IA del pipeline · responde "<strong>¿avanzamos?</strong>" y "<strong>¿qué hacer para crecer?</strong>" con datos reales del CRM.
            </small>
        </div>
        <?php if ((int) session()->get('id_roles') !== 5): ?>
        <div class="text-end" style="min-width:230px;">
            <small class="text-muted">Consumo del mes (compartido con OTTO financiero)</small>
    ...
    <div class="row g-3 mb-4">
        <?php foreach ($presets as $key => $p):
            $esEstrella = $key === 'plan_crecimiento';
        ?>
        <div class="col-md-6 col-lg-4">
            <div class="card preset-card h-100 <?= $esEstrella ? 'estrella' : '' ?>">
                <div class="card-body">
                    <div class="d-flex align-items-start gap-2 mb-2">
                        ...
                        <div class="flex-grow-1">
                            <h6 class="card-title mb-1 fw-bold">
                                ...
                                <?php if ($esEstrella): ?>
                                    <span class="badge bg-warning text-dark" style="font-size:0.6rem;">SÍNTESIS</span>
                                <?php endif; ?>
                            </h6>
                    ...
                    <form method="post" action="<?= base_url('crm/asesor-ia/analizar') ?>">
                        <?= csrf_field() ?>
    ...
    <div class="alert alert-info py-2 small">
        ...
        OTTO compara <strong>snapshots semanales</strong> para responder si avanzamos. Asegúrate de generar uno cada lunes en
        <a href="<?= base_url('crm/snapshots') ?>">Snapshots</a> para que el análisis sea más preciso.
    </div>
        ...
        <div class="card-body p-0">
            <?php if (empty($historial)): ?>
                <div class="text-center text-muted py-4 small">
                    Aún no hay análisis. Lanza uno desde las tarjetas de arriba o usa el widget flotante.
                </div>
            <?php else: ?>
                <table class="table table-sm mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?= base_url('crm/asesor-ia/ver/' . $h['id_conversacion']) ?>" class="text-decoration-none">
                                    <?= esc($h['titulo']) ?>
                                </a>
                            </td>
                            <td><small><?= esc($h['creado_por'] ?? '—') ?></small></td>
                            <td class="text-center text-nowrap">
                                <a href="<?= base_url('crm/asesor-ia/ver/' . $h['id_conversacion']) ?>"
                                   class="btn btn-sm btn-outline-primary" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="<?= base_url('crm/asesor-ia/eliminar/' . $h['id_conversacion']) ?>"
